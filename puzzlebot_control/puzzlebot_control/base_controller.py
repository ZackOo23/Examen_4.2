#!/usr/bin/env python3
"""
Base Controller for PuzzleBot — v2 (fixed)
───────────────────────────────────────────
Fixes aplicados:
  FIX 1: Indentación rota en __init__ — cmd_pub y odom_sub estaban fuera
          del constructor, causando que self.cmd_pub nunca existiera.
  FIX 2: Parámetro odom_topic configurable desde launch/CLI.
          Por defecto usa /odom (Gazebo). Para hardware real pasa:
            --ros-args -p odom_topic:=/odom_hw
"""
import rclpy
import numpy as np
...
class BaseController(Node):
    ...
    def __init__(self, node_name: str):
        ...
        # FIX 1+2: estos declare/get DEBEN estar aquí dentro, con indentación correcta
        self.declare_parameter('cmd_vel_topic', '/cmd_vel')
        self.declare_parameter('odom_topic',    '/odom')   # override con /odom_hw para hardware
        ...
        # FIX 1: Publishers — ahora correctamente dentro de __init__
        ...
        # FIX 1: Subscribers — ahora correctamente dentro de __init__
    ...
    def _goal_cb(self, msg: PoseStamped):
        ...
        self.on_new_goal()
        self.get_logger().info(
            f'[{self.CONTROLLER_NAME}] Goal received: ({self.goal_x:.2f}, {self.goal_y:.2f})')
    ...
    def _publish_state(self, v, w, V, dist, angle_err, dist_dot, angle_dot, s_v, s_w):
        state = {
            ...
            's_v':        round(s_v, 5),
            's_w':        round(s_w, 5),
            'gx':         round(self.goal_x, 3),
            'gy':         round(self.goal_y, 3),
            'wp':         self.wp_index,
